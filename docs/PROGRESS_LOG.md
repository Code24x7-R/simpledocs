...
## 2026-08-26 — [FEATURE] PWA file association for .sdjson

### Summary

Implemented PWA file handler support so that double-clicking a `.sdjson` file on Windows launches SimpleDocs (when installed as a PWA) and opens the document. Uses the Web App Launch Queue API (`window.launchQueue`) with a `file_handlers` entry in the manifest.

### Approach

Chose PWA `file_handlers` over a custom protocol handler because:
- No URL length limits (file arrives as a `File` object, not a URL param)
- No registry hacking or external launcher scripts needed
- True native double-click-to-open UX on Windows 10/11 (Chromium browsers)

Rejected protocol handler (`simpledocs://`) because encoding documents in URLs is capped at ~30KB (same constraint as existing `#doc=` share links).

### Changes
- `public/manifest.json` — new PWA manifest with `file_handlers` for `.sdjson` (`application/json` MIME)
- `public/icon-192.png`, `public/icon-512.png` — PWA icons generated by `scripts/generateIcons.mjs`
- `src/hooks/useFileLaunch.ts` — hook that consumes `launchQueue`, validates DocState shape, and calls `loadDocument`
- `src/hooks/useFileLaunch.test.tsx` — 7 tests covering: no-op in tab mode, consumer registration, valid doc loading, invalid shape skip, JSON parse failure, legacy `pages[]` format, and multi-file launch
- `src/types/launch-queue.d.ts` — TypeScript declarations for the experimental Launch Queue API
- `index.html` — added `<link rel="manifest">` and `theme-color` meta
- `src/App.tsx` — wired in `useFileLaunch()`

### Results

7 new tests pass, lint clean, type-check clean, build succeeds. Reuses existing `loadDocument` (migration + sanitization). Validates same shape as Drive open handler: `id`, `settings`, (`content` || `pages`).
